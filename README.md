...
| — | `HTTP_TIMEOUT` | `20`(초) | 게시판 응답 대기 시간 |
...
> **GitHub 무료 러너의 예약 실행은 실제로는 상당히 밀립니다.**
> `*/30` 으로 설정해도 실측은 2~5시간 간격이었습니다(3주간 308회).
> 30분 단위의 정확한 주기가 필요하면 서버나 라즈베리파이에서 `watch` 로 돌리세요.
...
- 학교 서버가 가끔 느려 응답이 지연됩니다. 접속 실패는 2초·4초 간격으로 최대 3번까지
  다시 시도하므로, 한 번의 지연으로 그 회차를 놓치지 않습니다.
